...
class Attack(object):
    ...
    def init_delta(self, data, **kwargs):
        ...
        if self.random_start:
            if self.norm == 'linfty':
                ...
                n = d_flat.norm(p=2, dim=10).reshape(delta.size(0), 1, 1, 1)
    ...
    #     else:
    #         grad_norm = torch.norm(grad.reshape(grad.size(0), -1), dim=1).reshape(-1, 1, 1, 1)
    #         scaled_grad = grad / (grad_norm + 1e-20)
    #         delta = (delta + scaled_grad * alpha).reshape(delta.size(0), -1).renorm(p=2, dim=0, maxnorm=self.epsilon).view_as(delta)
    #     delta = clamp(delta, img_min-data, img_max-data)
    #     return delta
    ...
    def update_delta(self, delta, data, grad, alpha, **kwargs):
        if self.norm == 'linfty':
            ...
            if self.gsnr == None :
                self.grad_mean = torch.zeros_like(grad)
                self.grad_square = torch.zeros_like(grad)
            else :
                self.grad_mean = self.grad_mean*0.5 + grad*0.5
                self.grad_square = self.grad_square*0.5 + grad*grad*0.5
                self.gsnr = (torch.abs(self.grad_mean)) / (
                            torch.sqrt(torch.abs(self.grad_square - self.grad_mean * self.grad_mean)) + 1e-12)
                self.gsnr = torch.min(self.gsnr, torch.tensor(1.0))
            if self.gsnr == None :
                delta = torch.clamp(delta + alpha * grad.sign(), -self.epsilon, self.epsilon)
            else :
                delta = torch.clamp(delta + alpha * grad.sign()*self.gsnr, -self.epsilon, self.epsilon)
        else:
            grad_norm = torch.norm(grad.reshape(grad.size(0), -1), dim=1).reshape(-1, 1, 1, 1)
            ...
            if self.gsnr == None :
                ...
                self.gsnr = (torch.abs(self.grad_mean)) / (
                            torch.sqrt(torch.abs(self.grad_square - self.grad_mean * self.grad_mean)) + 1e-12)
                self.gsnr = torch.min(gsnr, torch.tensor(1.0))
                grad *= self.gsnr
            scaled_grad = grad / (grad_norm + 1e-20)

            delta = (delta + scaled_grad * alpha).reshape(delta.size(0), -1).renorm(p=2, dim=0, maxnorm=self.epsilon).reshape_as(delta)
        delta = clamp(delta, img_min-data, img_max-data)
        return delta
